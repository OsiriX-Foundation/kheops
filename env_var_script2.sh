...
if /usr/bin/find "/etc/nginx/kheops2/" -mindepth 1 -maxdepth 1 -type f -print -quit 2>/dev/null | read v; then
    echo >&3 "$0: /etc/nginx/kheops2/ is not empty, will attempt to perform configuration"

    echo >&3 "$0: Looking for .conf file in /etc/nginx/kheops2/"
    find "/etc/nginx/kheops2/" -follow -type f -print | sort -n | while read -r f; do
        case "$f" in
            *.conf)
                remplace_in_file ${f}
                ;;
...
cp "${LETSENCRYPT_DIR:-/etc/letsencrypt}/live/$roothost/privkey.pem" /usr/share/nginx/certificates/privkey.pe
