...
public class SendingResource
{
            ...
            try {
                if (kheopsPrincipal.getScope() == ScopeType.ALBUM) {
                    ...
                    if (kheopsPrincipal.hasAlbumPermission(ADD_SERIES, albumID)) {
                        Sending.putSeriesInAlbum(fooHashMap, context, kheopsPrincipal, albumID, studyInstanceUID, seriesInstanceUID, kheopsPrincipal.getKheopsLogBuilder());
                        return Response.status(CREATED).build();
                    } else {
                        final ErrorResponse errorResponse = new ErrorResponse.ErrorResponseBuilder()
                                .message(AUTHORIZATION_ERROR)
    ...
    public Response appropriateStudy(@PathParam(StudyInstanceUID) @UIDValidator String studyInstanceUID,
        ...
        try {
            if (kheopsPrincipal.getScope() == ScopeType.ALBUM) {
                final String albumID = kheopsPrincipal.getAlbumID();
                if (kheopsPrincipal.hasAlbumPermission(ADD_SERIES, albumID)) {
                    Sending.putStudyInAlbum(fooHashMap, context, kheopsPrincipal, albumID, studyInstanceUID, albumId, false, kheopsPrincipal.getKheopsLogBuilder());
                } else {
                    final ErrorResponse errorResponse = new ErrorResponse.ErrorResponseBuilder()
                            .message(AUTHORIZATION_ERROR)
                            .detail("No write access with this credential")
    ...
    public Response putStudyInAlbum(@SuppressWarnings("RSReferenceInspection") @PathParam(ALBUM) String albumId,
                                    ...
                                    @QueryParam(ALBUM) String fromAlbumId,
                                    @QueryParam(INBOX) Boolean fromInbox)
            throws AlbumNotFoundException, UserNotMemberException, StudyNotFoundException {
        ...
        try {
            Sending.putStudyInAlbum(fooHashMap, context, kheopsPrincipal, albumId, studyInstanceUID, fromAlbumId, fromInbox, kheopsPrincipal.getKheopsLogBuilder());
