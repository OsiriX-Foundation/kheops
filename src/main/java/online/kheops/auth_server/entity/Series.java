...
@NamedQueries({
        ...
        @NamedQuery(name = "Series.findByStudyUIDFromInbox",
                query = "SELECT s FROM User u JOIN u.albumUser au JOIN au.album a JOIN a.albumSeries alS JOIN alS.series s WHERE a = u.inbox AND u=:"+USER+" AND s.study.studyInstanceUID = :StudyInstanceUID AND s.seriesInstanceUID = :"+SERIES_UID),
        @NamedQuery(name = "Series.findByStudyUIDFromAlbum",
                query = "SELECT s FROM Album a JOIN a.albumSeries alS JOIN alS.series s WHERE :"+ALBUM+" = a AND s.study.studyInstanceUID = :"+STUDY_UID+" AND s.seriesInstanceUID = :SeriesInstanceUID"),
        @NamedQuery(name = "Series.findBySeriesUIDAndStudyUIDAndUser",
                query = "SELECT s FROM User u JOIN u.albumUser au JOIN au.album a JOIN a.albumSeries alS JOIN alS.series s WHERE u=:"+USER+" AND s.study.studyInstanceUID = :StudyInstanceUID AND s.seriesInstanceUID = :"+SERIES_UID),
        @NamedQuery(name = "Series.findBySeriesUIDAndStudyUIDAndUserWithSharePermission",
                query = "SELECT s FROM User u JOIN u.albumUser au JOIN au.album a JOIN a.albumSeries alS JOIN alS.series s WHERE (a = u.inbox or au.admin = true or a.userPermission.sendSeries = true)AND u=:"+USER+" AND s.study.studyInstanceUID = :StudyInstanceUID AND s.seriesInstanceUID = :"+SERIES_UID),
