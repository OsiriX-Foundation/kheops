/*
 * This file is generated by jOOQ.
 */
package online.kheops.auth_server.generated.tables.records;


import java.sql.Timestamp;

import javax.annotation.Generated;

import online.kheops.auth_server.generated.tables.Capabilities;

import org.jooq.Field;
import org.jooq.Record1;
import org.jooq.Record16;
import org.jooq.Row16;
import org.jooq.impl.UpdatableRecordImpl;


/**
 * This class is generated by jOOQ.
 */
@Generated(
    value = {
        "http://www.jooq.org",
        "jOOQ version:3.11.2"
    },
    comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class CapabilitiesRecord extends UpdatableRecordImpl<CapabilitiesRecord> implements Record16<Long, String, Timestamp, Timestamp, Timestamp, Timestamp, Timestamp, String, String, Boolean, Boolean, Boolean, Boolean, Long, String, Long> {

    private static final long serialVersionUID = -1316957543;

    /**
     * Setter for <code>public.capabilities.pk</code>.
     */
    public void setPk(Long value) {
        set(0, value);
    }

    /**
     * Getter for <code>public.capabilities.pk</code>.
     */
    public Long getPk() {
        return (Long) get(0);
    }

    /**
     * Setter for <code>public.capabilities.id</code>.
     */
    public void setId(String value) {
        set(1, value);
    }

    /**
     * Getter for <code>public.capabilities.id</code>.
     */
    public String getId() {
        return (String) get(1);
    }

    /**
     * Setter for <code>public.capabilities.issued_at_time</code>.
     */
    public void setIssuedAtTime(Timestamp value) {
        set(2, value);
    }

    /**
     * Getter for <code>public.capabilities.issued_at_time</code>.
     */
    public Timestamp getIssuedAtTime() {
        return (Timestamp) get(2);
    }

    /**
     * Setter for <code>public.capabilities.updated_time</code>.
     */
    public void setUpdatedTime(Timestamp value) {
        set(3, value);
    }

    /**
     * Getter for <code>public.capabilities.updated_time</code>.
     */
    public Timestamp getUpdatedTime() {
        return (Timestamp) get(3);
    }

    /**
     * Setter for <code>public.capabilities.expiration_time</code>.
     */
    public void setExpirationTime(Timestamp value) {
        set(4, value);
    }

    /**
     * Getter for <code>public.capabilities.expiration_time</code>.
     */
    public Timestamp getExpirationTime() {
        return (Timestamp) get(4);
    }

    /**
     * Setter for <code>public.capabilities.not_before_time</code>.
     */
    public void setNotBeforeTime(Timestamp value) {
        set(5, value);
    }

    /**
     * Getter for <code>public.capabilities.not_before_time</code>.
     */
    public Timestamp getNotBeforeTime() {
        return (Timestamp) get(5);
    }

    /**
     * Setter for <code>public.capabilities.revoked_time</code>.
     */
    public void setRevokedTime(Timestamp value) {
        set(6, value);
    }

    /**
     * Getter for <code>public.capabilities.revoked_time</code>.
     */
    public Timestamp getRevokedTime() {
        return (Timestamp) get(6);
    }

    /**
     * Setter for <code>public.capabilities.title</code>.
     */
    public void setTitle(String value) {
        set(7, value);
    }

    /**
     * Getter for <code>public.capabilities.title</code>.
     */
    public String getTitle() {
        return (String) get(7);
    }

    /**
     * Setter for <code>public.capabilities.secret</code>.
     */
    public void setSecret(String value) {
        set(8, value);
    }

    /**
     * Getter for <code>public.capabilities.secret</code>.
     */
    public String getSecret() {
        return (String) get(8);
    }

    /**
     * Setter for <code>public.capabilities.read_permission</code>.
     */
    public void setReadPermission(Boolean value) {
        set(9, value);
    }

    /**
     * Getter for <code>public.capabilities.read_permission</code>.
     */
    public Boolean getReadPermission() {
        return (Boolean) get(9);
    }

    /**
     * Setter for <code>public.capabilities.appropriate_permission</code>.
     */
    public void setAppropriatePermission(Boolean value) {
        set(10, value);
    }

    /**
     * Getter for <code>public.capabilities.appropriate_permission</code>.
     */
    public Boolean getAppropriatePermission() {
        return (Boolean) get(10);
    }

    /**
     * Setter for <code>public.capabilities.download_permission</code>.
     */
    public void setDownloadPermission(Boolean value) {
        set(11, value);
    }

    /**
     * Getter for <code>public.capabilities.download_permission</code>.
     */
    public Boolean getDownloadPermission() {
        return (Boolean) get(11);
    }

    /**
     * Setter for <code>public.capabilities.write_permission</code>.
     */
    public void setWritePermission(Boolean value) {
        set(12, value);
    }

    /**
     * Getter for <code>public.capabilities.write_permission</code>.
     */
    public Boolean getWritePermission() {
        return (Boolean) get(12);
    }

    /**
     * Setter for <code>public.capabilities.user_fk</code>.
     */
    public void setUserFk(Long value) {
        set(13, value);
    }

    /**
     * Getter for <code>public.capabilities.user_fk</code>.
     */
    public Long getUserFk() {
        return (Long) get(13);
    }

    /**
     * Setter for <code>public.capabilities.scope_type</code>.
     */
    public void setScopeType(String value) {
        set(14, value);
    }

    /**
     * Getter for <code>public.capabilities.scope_type</code>.
     */
    public String getScopeType() {
        return (String) get(14);
    }

    /**
     * Setter for <code>public.capabilities.album_fk</code>.
     */
    public void setAlbumFk(Long value) {
        set(15, value);
    }

    /**
     * Getter for <code>public.capabilities.album_fk</code>.
     */
    public Long getAlbumFk() {
        return (Long) get(15);
    }

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Record1<Long> key() {
        return (Record1) super.key();
    }

    // -------------------------------------------------------------------------
    // Record16 type implementation
    // -------------------------------------------------------------------------

    /**
     * {@inheritDoc}
     */
    @Override
    public Row16<Long, String, Timestamp, Timestamp, Timestamp, Timestamp, Timestamp, String, String, Boolean, Boolean, Boolean, Boolean, Long, String, Long> fieldsRow() {
        return (Row16) super.fieldsRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Row16<Long, String, Timestamp, Timestamp, Timestamp, Timestamp, Timestamp, String, String, Boolean, Boolean, Boolean, Boolean, Long, String, Long> valuesRow() {
        return (Row16) super.valuesRow();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Long> field1() {
        return Capabilities.CAPABILITIES.PK;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field2() {
        return Capabilities.CAPABILITIES.ID;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field3() {
        return Capabilities.CAPABILITIES.ISSUED_AT_TIME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field4() {
        return Capabilities.CAPABILITIES.UPDATED_TIME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field5() {
        return Capabilities.CAPABILITIES.EXPIRATION_TIME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field6() {
        return Capabilities.CAPABILITIES.NOT_BEFORE_TIME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Timestamp> field7() {
        return Capabilities.CAPABILITIES.REVOKED_TIME;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field8() {
        return Capabilities.CAPABILITIES.TITLE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field9() {
        return Capabilities.CAPABILITIES.SECRET;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field10() {
        return Capabilities.CAPABILITIES.READ_PERMISSION;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field11() {
        return Capabilities.CAPABILITIES.APPROPRIATE_PERMISSION;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field12() {
        return Capabilities.CAPABILITIES.DOWNLOAD_PERMISSION;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Boolean> field13() {
        return Capabilities.CAPABILITIES.WRITE_PERMISSION;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Long> field14() {
        return Capabilities.CAPABILITIES.USER_FK;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<String> field15() {
        return Capabilities.CAPABILITIES.SCOPE_TYPE;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Field<Long> field16() {
        return Capabilities.CAPABILITIES.ALBUM_FK;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long component1() {
        return getPk();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component2() {
        return getId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component3() {
        return getIssuedAtTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component4() {
        return getUpdatedTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component5() {
        return getExpirationTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component6() {
        return getNotBeforeTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp component7() {
        return getRevokedTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component8() {
        return getTitle();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component9() {
        return getSecret();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean component10() {
        return getReadPermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean component11() {
        return getAppropriatePermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean component12() {
        return getDownloadPermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean component13() {
        return getWritePermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long component14() {
        return getUserFk();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String component15() {
        return getScopeType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long component16() {
        return getAlbumFk();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long value1() {
        return getPk();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value2() {
        return getId();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value3() {
        return getIssuedAtTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value4() {
        return getUpdatedTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value5() {
        return getExpirationTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value6() {
        return getNotBeforeTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Timestamp value7() {
        return getRevokedTime();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value8() {
        return getTitle();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value9() {
        return getSecret();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value10() {
        return getReadPermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value11() {
        return getAppropriatePermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value12() {
        return getDownloadPermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Boolean value13() {
        return getWritePermission();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long value14() {
        return getUserFk();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public String value15() {
        return getScopeType();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public Long value16() {
        return getAlbumFk();
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value1(Long value) {
        setPk(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value2(String value) {
        setId(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value3(Timestamp value) {
        setIssuedAtTime(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value4(Timestamp value) {
        setUpdatedTime(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value5(Timestamp value) {
        setExpirationTime(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value6(Timestamp value) {
        setNotBeforeTime(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value7(Timestamp value) {
        setRevokedTime(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value8(String value) {
        setTitle(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value9(String value) {
        setSecret(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value10(Boolean value) {
        setReadPermission(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value11(Boolean value) {
        setAppropriatePermission(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value12(Boolean value) {
        setDownloadPermission(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value13(Boolean value) {
        setWritePermission(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value14(Long value) {
        setUserFk(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value15(String value) {
        setScopeType(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord value16(Long value) {
        setAlbumFk(value);
        return this;
    }

    /**
     * {@inheritDoc}
     */
    @Override
    public CapabilitiesRecord values(Long value1, String value2, Timestamp value3, Timestamp value4, Timestamp value5, Timestamp value6, Timestamp value7, String value8, String value9, Boolean value10, Boolean value11, Boolean value12, Boolean value13, Long value14, String value15, Long value16) {
        value1(value1);
        value2(value2);
        value3(value3);
        value4(value4);
        value5(value5);
        value6(value6);
        value7(value7);
        value8(value8);
        value9(value9);
        value10(value10);
        value11(value11);
        value12(value12);
        value13(value13);
        value14(value14);
        value15(value15);
        value16(value16);
        return this;
    }

    // -------------------------------------------------------------------------
    // Constructors
    // -------------------------------------------------------------------------

    /**
     * Create a detached CapabilitiesRecord
     */
    public CapabilitiesRecord() {
        super(Capabilities.CAPABILITIES);
    }

    /**
     * Create a detached, initialised CapabilitiesRecord
     */
    public CapabilitiesRecord(Long pk, String id, Timestamp issuedAtTime, Timestamp updatedTime, Timestamp expirationTime, Timestamp notBeforeTime, Timestamp revokedTime, String title, String secret, Boolean readPermission, Boolean appropriatePermission, Boolean downloadPermission, Boolean writePermission, Long userFk, String scopeType, Long albumFk) {
        super(Capabilities.CAPABILITIES);

        set(0, pk);
        set(1, id);
        set(2, issuedAtTime);
        set(3, updatedTime);
        set(4, expirationTime);
        set(5, notBeforeTime);
        set(6, revokedTime);
        set(7, title);
        set(8, secret);
        set(9, readPermission);
        set(10, appropriatePermission);
        set(11, downloadPermission);
        set(12, writePermission);
        set(13, userFk);
        set(14, scopeType);
        set(15, albumFk);
    }
}
